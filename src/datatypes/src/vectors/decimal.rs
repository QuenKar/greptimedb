...
use crate::error::{self, CastTypeSnafu, PrecisionOrScaleSnafu, Result};
...
impl Decimal128Vector {
    ...
    pub fn with_precision_and_scale(self, precision: u8, scale: i8) -> Result<Self> {
        let array = self
            ...
            .context(PrecisionOrScaleSnafu { precision, scale })?;
        Ok(Self { array })
    }
    ...
    fn get(&self, index: usize) -> Value {
        ...
        match self.array.data_type() {
            ArrowDataType::Decimal128(precision, scale) => {
                ...
                Value::Decimal128(Decimal128::new(value, *precision, *scale))
            }
            _ => Value::Null,
        }
    }
...
impl Debug for Decimal128Vector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Decimal128Vector")
    ...
    fn next(&mut self) -> Option<Self::Item> {
        Some(self.iter.next().and_then(|v| {
            v.and_then(|v| match self.data_type {
                ArrowDataType::Decimal128(precision, scale) => {
                    Some(Decimal128::new(v, *precision, *scale))
                }
                _ => None,
            })
        }))
